...
def launch(launch_descriptor, argv):
    ...
    package = 'composition'
    ld.add_process(
        cmd=[get_executable_path(package_name=package, executable_name='dlopen_composition'),
        ''`ros2 pkg prefix detection`/lib/libposition_estimation_component.so','`ros2 pkg prefix detection`/lib/libtarget_publisher_component.so'],
        name='position_estimation_target_publisher_node',
        exit_handler=restart_exit_handler,
    )
